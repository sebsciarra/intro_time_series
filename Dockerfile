...
# and `pip install intro_time_series` using an artifact store. Faster and more robust.
...
SHELL ["mamba", "run", "-n", "intro_time_series", "/bin/bash", "-c"]

# Build package
RUN tox -e build
RUN mamba env export -n intro_time_series -f environment.docker.yml
...
SHELL ["conda", "run", "-n", "intro_time_series", "/bin/bash", "-c"]
...
# ENTRYPOINT ["conda", "run", "--no-capture-output", "-n", "intro_time_series", "YOUR_CONSOLE_SCRIPT"]
